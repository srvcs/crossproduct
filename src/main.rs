use srvcs_crossproduct::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-crossproduct listening");
    let deps = Deps {
        floatmultiply_url: cfg.floatmultiply_url.clone(),
        floatsubtract_url: cfg.floatsubtract_url.clone(),
    };
